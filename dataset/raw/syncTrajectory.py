...
def readTag(file_list):
	...
	for filename in sorted_file_list:
		with open(filename, "rb") as logfile:
			...
			results[tval] = {'meta': meta, 'diag_anc0': diag0, 'diag_anc1': diag1, 'diag_anc2': diag2, 'resp_anc0': resp0, 'resp_anc1': resp1, 'resp_anc2': resp2}
	return results
...
def readAnch(file_list):
	...
	for filename in sorted_file_list:
		with open(filename, "rb") as logfile:
			...
			if tval in results:
				print("======== Duplicated TVAL ========")
			results[tval] = {'meta': meta, 'diag0': diag0, 'diag1': diag1, 'poll': poll, 'final': final}
	return results
...
while (len(tag0_tvals) > 0) and (len(anc0_tvals) > 0) and (len(anc1_tvals) > 0) and (len(anc2_tvals) > 0):
	...
	sample = [-1]*7
	sample_empty = True
	if (tag0_valid_anc0 > 0) and (cur_seq_anc0 == anc0_seq0):
		seq_num = cur_seq_anc0
		sample_empty = False
	if (tag0_valid_anc1 > 0) and (cur_seq_anc1 == anc1_seq0):
		seq_num = cur_seq_anc1
		sample_empty = False
	if (tag0_valid_anc2 > 0) and (cur_seq_anc2 == anc2_seq0):
	...
	# if (tag0_valid_anc0 > 0) and (seq_num == anc0_seq0):
	# 	sample[1] = anc0_seq0
	# 	sample[4] = anc0_tvals[0] - tag0_tvals[0]
	# 	sample_empty = False
	# if (tag0_valid_anc1 > 0) and (seq_num == anc1_seq0):
	# 	sample[2] = anc1_seq0
	# 	sample[5] = anc1_tvals[0] - tag0_tvals[0]
	# 	sample_empty = False
	# if (tag0_valid_anc2 > 0) and (seq_num == anc2_seq0):
	# 	sample[3] = anc2_seq0
	# 	sample[6] = anc2_tvals[0] - tag0_tvals[0]
	# 	sample_empty = False
	# if not sample_empty:
	# 	sample[0] = seq_num
	# 	sync_data.append(sample)

	sample = {'gt': None, 'seq': -1, 'anc0': None, 'anc1': None, 'anc2': None}
	...
	if (tag0_valid_anc0 > 0) and (seq_num == anc0_seq0):
		...
		sample['anc0'] = {'poll': anc0['poll'], 'fp_poll': fp_poll_anc0, 'resp': tag['resp_anc0'], 'fp_resp': fp_resp_anc0, 'final': anc0['final'], 'fp_final': fp_final_anc0, 'range': range_anc0, 'seq': anc0_seq0, 'tval': anc0_tvals[0]}

	if (tag0_valid_anc1 > 0) and (seq_num == anc1_seq0):
		anc1 = anc1_data[anc1_tvals[0]]
		fp_poll_anc1 = float((anc1['diag0'])[0])/64.0
		fp_final_anc1 = float((anc1['diag1'])[0])/64.0
		sample['anc1'] = {'poll': anc1['poll'], 'fp_poll': fp_poll_anc1, 'resp': tag['resp_anc1'], 'fp_resp': fp_resp_anc1, 'final': anc1['final'], 'fp_final': fp_final_anc1, 'range': range_anc1, 'seq': anc1_seq0, 'tval': anc1_tvals[0]}

	if (tag0_valid_anc2 > 0) and (seq_num == anc2_seq0):
		anc2 = anc2_data[anc2_tvals[0]]
		fp_poll_anc2 = float((anc2['diag0'])[0])/64.0
		fp_final_anc2 = float((anc2['diag1'])[0])/64.0
		sample['anc2'] = {'poll': anc2['poll'], 'fp_poll': fp_poll_anc2, 'resp': tag['resp_anc2'], 'fp_resp': fp_resp_anc2, 'final': anc2['final'], 'fp_final': fp_final_anc2, 'range': range_anc2, 'seq': anc2_seq0, 'tval': anc2_tvals[0]}

	if not sample_empty:
		if tag0_tvals[0] in tag0_gt:
			sample['gt'] = tag0_gt[tag0_tvals[0]]
			print("-------")
		else:
			print("Lost groundtruth")
		sample['seq'] = seq_num
		sync_data[tag0_tvals[0]] = sample

	tag0_tvals.pop(0)

dataset = {'data': sync_data, 'anc0_loc': anc0_loc, 'anc1_loc': anc1_loc, 'anc2_loc': anc2_loc}
